...
router.get("/download", async (req, res) => {
  try {
    ...
    res.redirect(303, aboutInfo.resumeUrl)
  } catch (error) {
...
// Download resume through server with signed URL
router.get("/resume/download", async (req, res) => {
  try {
    ...
    if (aboutInfo.resumeUrl.includes("cloudinary.com")) {
      ...
      const downloadUrl = `https://api.cloudinary.com/v1_1/${process.env.CLOUDINARY_CLOUD_NAME}/raw/upload/${publicId}?api_key=${process.env.CLOUDINARY_API_KEY}&timestamp=${signParams.timestamp}&signature=${signature}&resource_type=raw`
